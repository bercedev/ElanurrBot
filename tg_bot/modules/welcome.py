    ...
    if should_welc:
        ...
        for new_mem in new_members:
            ...
            if new_mem.id == OWNER_ID:
                update.effective_message.reply_text("Efendi evde, hadi bu partiyi başlatalım!")
                continue
...
def goodbye(bot: Bot, update: Update, args: List[str]):
    ...
    if len(args) == 0 or args[0] == "noformat":
        ...
        update.effective_message.reply_text(
            "Bu sohbetin veda ayarı şu şekilde ayarlanmıştır: `{}`. \ N * Hoşçakal mesajı "
            "(not filling the {{}}) is:*".format(pref),
            parse_mode=ParseMode.MARKDOWN)
        ...
        if args[0].lower() in ("on", "yes"):
            ...
            update.effective_message.reply_text("İnsanlar gittiğinde sessiz olacağım!")
            ...
            # idek what you're writing, say yes or no
            update.effective_message.reply_text("Lütfen sadece 'on/yes' or 'off/no' ile kullanın.")
...
def reset_welcome(bot: Bot, update: Update) -> str:
    ...
    update.effective_message.reply_text("Hoş geldiniz mesajı başarıyla varsayılana sıfırlandı!")
    return "<b>{}:</b>" \
           "\n#RESET_WELCOME" \
           "\n<b>Admin:</b> {}" \
...
def reset_goodbye(bot: Bot, update: Update) -> str:
    ...
    update.effective_message.reply_text("Hoşçakal mesajını başarıyla varsayılana sıfırlandı")
    return "<b>{}:</b>" \
           "\n#RESET_GOODBYE" \
           "\n<b>Admin:</b> {}" \
...
WELC_HELP_TXT = "Grubunuzun hoşgeldin / hoşçakal mesajları birçok şekilde kişiselleştirilebilir. Eğer mesajları istiyorsan" \
                " tek tek oluşturulmak için, varsayılan karşılama mesajı gibi, * bu * değişkenleri kullanabilirsiniz:\n" \
                ...
                "buttons.\n" \
                "Kurallarınıza bağlanan bir düğme oluşturmak için şunu kullanın: "[Kurallar](buttonurl://t.me/Elanurrbot?Start=group_id)`. " \
